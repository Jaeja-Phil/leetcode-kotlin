...
fun main() {
    ...
    fun totalCostToHireKWorkers(costs: IntArray, k: Int, candidates: Int): Long {
        var startIdx = 0
        var endIdx = costs.lastIndex
        val minHeap = PriorityQueue<Node> { a, b ->
            // if the values are the same, we break the tie by the smaller index
            if (a.value == b.value) {
                a.idx - b.idx
            } else {
                // otherwise, we sort by the value
                a.value - b.value
            }
        }
        // start adding candidates to the min heap from the start and end of the costs array
        while (startIdx < candidates) {
            minHeap.add(Node(costs[startIdx], startIdx))
            startIdx++
        }
        while (startIdx <= endIdx && endIdx > costs.lastIndex - candidates) {
            minHeap.add(Node(costs[endIdx], endIdx))
            endIdx--
        }
        ...
        while (sessions < k) {
            ...
            val currNodeIdx = node.idx
            if (startIdx in (currNodeIdx + 1)..endIdx) {
                minHeap.add(Node(costs[startIdx], startIdx))
                startIdx++
            } else if (endIdx in startIdx..currNodeIdx) {
                minHeap.add(Node(costs[endIdx], endIdx))
                endIdx--
            }
            sessions++
    ...
    val costs1 = intArrayOf(17, 12, 10, 2, 7, 2, 11, 20, 8)
    val k1 = 3
    val candidates1 = 4
    println(totalCostToHireKWorkers(costs1, k1, candidates1)) // 11

    val costs2 = intArrayOf(1, 2, 4, 1)
    val k2 = 3
    val candidates2 = 3
    println(totalCostToHireKWorkers(costs2, k2, candidates2)) // 4
}
