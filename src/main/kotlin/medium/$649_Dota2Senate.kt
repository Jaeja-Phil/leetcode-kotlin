 ...
 */
fun main() {
    val res1 = `Dota 2 Senate`("RD")
    println(res1) // Radiant

    val res2 = `Dota 2 Senate`("RDD")
    println(res2) // Dire
}

fun `Dota 2 Senate`(senate: String): String {
    /**
    ...
    /**
     * loop through the senate string and add the index of each senator to the corresponding queue
     */
    senate.forEachIndexed { idx, char ->
        when (char) {
            'R' -> radiantQueue.add(idx)
            'D' -> direQueue.add(idx)
            else -> throw IllegalArgumentException("Invalid party: $char")
        }
    }

    /**
     * start "voting" until one of the queue is empty
     */
    while (radiantQueue.isNotEmpty() && direQueue.isNotEmpty()) {
        /**
         * get the index of the first senator in each queue
         */
        val radiantIdx = radiantQueue.removeFirst()
        val direIdx = direQueue.removeFirst()

        /**
         * if the index of the first senator in the Radiant queue is less than the index of the first senator in the
         * Dire queue, it means the Radiant senator is ahead of the Dire senator in the senate string, so the Radiant
         * senator can ban the Dire senator's right to vote
         */
        if (radiantIdx < direIdx) {
            radiantQueue.add(radiantIdx + senate.length)
        } else {
            direQueue.add(direIdx + senate.length)
        }
    }

    /**
     * party with remaining senator wins
     */
